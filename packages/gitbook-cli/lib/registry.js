...
function installVersion(version, forceInstall) {
    return resolveVersion(version)
        ...
        .then(function (tmpDir) {
            var gitbookRoot = path.resolve(tmpDir, 'node_modules/@jeff-tian/gitbook');
            ...
            if (!tags.isValid(version)) {
                // throw 'Invalid GitBook version, should satisfies ' + config.GITBOOK_VERSION;
                console.warn('Invalid GitBook version, should satisfies ' + config.GITBOOK_VERSION + ', but got ' + version);
            }
